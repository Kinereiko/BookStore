...
public class ShoppingCartServiceImpl implements ShoppingCartService {
    ...
    public ShoppingCartDto addCartItem(CartItemRequestDto requestDto) {
        ...
        CartItem cartItemExist = cartItemRepository.findByShoppingCartIdWhereBookId(user.getId(), requestDto.getBookId());
        if (cartItemExist != null) {
            System.out.println("Work");
            return updateCartItemById(cartItemExist.getId(), cartItemExist.getQuantity() + requestDto.getQuantity());
        }
    ...
    public ShoppingCartDto updateCartItemById(Long id, int quantity) {
        CartItem cartItem = cartItemRepository.findById(id)
                .orElseThrow(() -> new EntityNotFoundException("Can't find cart item with id: " + id));
    ...
    public User getAuthenticatedUser() {
        ...
        if (authentication != null && authentication.getPrincipal() instanceof UserDetails) {
            return userRepository.findWithRolesByEmail(((UserDetails) authentication.getPrincipal()).getUsername()).get();
        }
        return null;
    }
}
