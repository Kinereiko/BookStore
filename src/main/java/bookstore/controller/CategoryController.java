...
public class CategoryController {
    ...
    public CategoryDto findById(@PathVariable Long id) {return  categoryService.findById(id);}

    @PreAuthorize("hasRole('ROLE_ADMIN')")
    @PostMapping
    @Operation(summary = "Create a category", description = "Create a new category")
    public CategoryDto save(@RequestBody @Valid CategoryRequestDto requestDto) {return categoryService.save(requestDto);}

    @PreAuthorize("hasRole('ROLE_ADMIN')")
    @PutMapping("/{id}")
    @Operation(summary = "Update the category", description = "Update information about the category by id")
    ...
    public void delete(@PathVariable Long id) {categoryService.deleteById(id);}
